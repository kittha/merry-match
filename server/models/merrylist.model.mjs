...
export const getMerryLists = async (userId) => {
  try {
    ...
    profiles.racial_preferences, profiles.meeting_interests, match_status.status, match_status.user_id_1, match_status.user_id_2
    FROM profiles
    LEFT JOIN
    ...
    WHERE
    match_status.user_id_1 = $1 AND (match_status.status = 'merry_match' OR match_status.status = 'merry');
      `,
      [userId]
    );
